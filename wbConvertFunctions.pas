unit wbConvertFunctions;

interface

implementation
uses Classes, SysUtils, StrUtils, Windows, wbInterface, frmViewMain;
...
previousrec, previousrec2: IwbContainerElementRef;
...
begin
  Result := rec._File.FileName;
  Result := Copy(Result, 1, (LastDelimiter('.', Result) - 1)) + '_';
  Result := Result + elementnewrec + '_' + Copy(rec.FormID.ToString, 3, 6);
  Result := Result + '_' + rec.EditorID;
end;
...
function AsContainer(const element: IwbElement): IwbContainerElementRef;
var
iContainer: IwbContainerElementRef;
begin
  if Supports(element, IwbContainerElementRef, iContainer) then
    Result := iContainer
  else
    raise Exception.Create('Container not supported');
end;

function AddAlias(const rec: IwbMainRecord; const val: String): String;
var
rRef: IwbMainRecord;
rContainer, rAlias: IwbElement;
iContainer, iContainerAlias: IwbContainerElementRef;
s: String;
begin
  /// ElementValueString needs to be formatted to formid
  rContainer := rec.ElementByPath['Aliases'];
  rRef := rec._File.RecordByFormID[TwbFormID.FromStr(val), True, True];
  if Assigned(rContainer) then
    rAlias := rContainer.Assign(MaxInt, Nil, False)
  else
  begin
    rContainer := rec.Add('Aliases', True);

    if Supports(rContainer, IwbContainerElementRef, iContainer) then
      rAlias := iContainer.Elements[0];
  end;

  if Assigned(rAlias) and Supports(rAlias, IwbContainerElementRef, iContainerAlias) then
  begin
    iContainerAlias.Add('ALST', True).EditValue := IntToStr(iAliasCounter);
    if Assigned(rRef) then
    begin
      if rRef._File.FileName = rec._File.FileName then
      begin
        /// Set Target May have to change element path
        s := rRef.Signature;
        iContainerAlias.Add('ALFR', True).EditValue := val;
      end;
    end;
  end;
...
function CheckForErrors(element: IwbContainerElementRef): String;
var
i: Integer;
elementcur: IwbElement;
iContainer: IwbContainerElementRef;
begin
	for i := 0 to element.ElementCount - 1 do
	begin
    elementcur := element.Elements[i];
    Result := elementcur.Check;

    if Result <> '' then
    begin
      if AnsiPos('Record Header', elementcur.Path) = 0 then
      begin
        Result := formatelementpath(elementcur.Path) + #13#10 + Result;
        Exit;
      end;
    end;

    if Supports(elementcur, IwbContainerElementRef, iContainer) then
    begin
      if iContainer.ElementCount > 0 then
        Result := CheckForErrors(iContainer);
    end;
	end;
end;

function ExitSequence(var form: TfrmMain; const _rec: IwbMainRecord; const _cont: IwbContainerElementRef; const _formid: String; const _path: String; const _value: String; const _integer: String; const _flag: String): Integer;
begin
	form.AddMessage('FAILED');
  form.AddMessage('PATH     : ' + _path);
  form.AddMessage('VALUE    : ' + _value);
  form.AddMessage('INTEGER  : ' + _integer);
  form.AddMessage('ISFLAG   : ' + _flag);
  if Assigned(_cont) then form.AddMessage('CONTPATH : ' + Copy(formatelementpath(_cont.Path), 6, MaxInt))
  else form.AddMessage('Container not Assigned');
  if Assigned(_rec) then form.AddMessage(_rec.Name)
  else form.AddMessage('Record not Assigned');
...
function RetrieveIndexPath(var form: TfrmMain; s: String): String;
var
i: Integer;
begin
  i := ansipos(']\[', s);
  ...
  if s = '' then
  begin
    form.AddMessage('Warning: RetrieveIndexPath returned nothing');
    s := '0'
  end;
  Result := s;
end;
...
procedure CreateElementQuick1(var form: TfrmMain; const rec: IwbMainRecord; const elementpathstring: String; const elementvaluestring: String);
var
subrec: IwbElement;
iContainer: IwbContainerElementRef;
begin
  subrec := rec.Add(elementpathstring, True);
  try
    ...
    begin
      ...
      begin
        form.AddMessage(Copy(E.Message, 12, MaxInt));
        subrec.EditValue := elementvaluestring;
        Exit;
      end;
    end;
  ...
  if Supports(subrec, IwbContainerElementRef, iContainer) then
    previousrec := iContainer;
end;

function FileByLoadOrder(var Files: TDynFiles; loadOrder: Integer): IwbFile;
var
  i: Integer;
begin
  Result := nil;
  ...
  if Result = nil then
    raise Exception.Create('File not found with loadorder: ' + IntToStr(loadOrder));
end;

////////////////////////////////////////////////////////////////////////////////
///  Procedure for Creating and modifying Elements
////////////////////////////////////////////////////////////////////////////////
function CreateElement(var Files: TDynFiles; var iFileFallout4ESM: IwbFile; var form: TfrmMain; rec: IwbMainRecord; originalloadorder: String;
                       fileloadorder: String; elementpathstring: String;
                       elementvaluestring: String; elementinteger: integer;
                       elementisflag: String): Integer;
var
subrec: IwbElement;
subrec_container: IwbContainerElementRef;
...
begin
  ...
  if rec.Signature = 'REGN' then
    if elementpathstring = 'WNAM' then
      if elementvaluestring = '' then
        elementpathstring := '';

  if rec.Signature = 'WATR' then
  ...
  begin
    if ((previousrec.Name = 'Effects') OR (previousrec.Name = 'Conditions')) then
      elementpathstring := '';
    if not Assigned(previousrec) then elementpathstring := '';
    if previousrec.Name = 'CTDA - ' then
      subrec_container := previousrec;
    if previousrec.Container.Name = 'CTDA - ' then
      subrec_container := AsContainer(previousrec.Container);
    if Assigned(subrec_container) then
    begin
      if subrec_container.ElementEditValues['Function'] = 'GetWantBlocking' then
      begin
      ...
      if subrec_container.ElementEditValues['Function'] = 'HasPerk' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'HasPerk' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementpathstring := '';
      if subrec_container.ElementEditValues['Function'] = 'GetIsID' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetIsID' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetIsReference' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetIsReference' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetItemCount' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetItemCount' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetVATSValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementpathstring := '';
      if subrec_container.ElementEditValues['Function'] = 'GetVATSValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetVMQuestVariable' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          ...
          begin
            elementpathstring := '';
            subrec_container.ElementEditValues['Function'] := 'GetWantBlocking';
          end;
        end;
      if subrec_container.ElementEditValues['Function'] = 'GetVMQuestVariable' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetInCell' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetInCell' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetDead' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetDead' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetLocked' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetLocked' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetDisabled' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetDisabled' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetInWorldspace' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          if elementvaluestring = 'NULL - Null Reference [00000000]' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetUnconscious' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetUnconscious' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetDeadCount' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          if elementvaluestring = 'NULL - Null Reference [00000000]' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetObjectiveCompleted' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetObjectiveCompleted' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetObjectiveDisplayed' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetObjectiveDisplayed' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetDestructionStage' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetDestructionStage' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetPos' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetPos' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetGlobalValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetGlobalValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetIsCreatureType' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementvaluestring := '';
          elementinteger := 5;
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetIsCreatureType' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetEquipped' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetEquipped' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementvaluestring := '';
          elementinteger := 5;
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'IsInCombat' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementvaluestring := '';
          elementinteger := 5;
      ...
      if subrec_container.ElementEditValues['Function'] = 'IsInCombat' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetIsUsedItemType' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'GetIsUsedItemType' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetStage' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          if elementvaluestring = 'NULL - Null Reference [00000000]' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetQuestRunning' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          if elementvaluestring = 'NULL - Null Reference [00000000]' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetQuestCompleted' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          if elementvaluestring = 'NULL - Null Reference [00000000]' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetFactionReaction' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          if elementvaluestring = 'NULL - Null Reference [00000000]' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetStageDone' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
          if elementvaluestring = 'NULL - Null Reference [00000000]' then
      ...
      if subrec_container.ElementEditValues['Function'] = 'IsWeaponSkillType' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementvaluestring := '';
          elementinteger := 5;
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetPermanentValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetPermanentValue' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'GetIsObjectType' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
        begin
          elementinteger := 5;
      ...
      if subrec_container.ElementEditValues['Function'] = 'GetIsObjectType' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
          elementinteger := 6;
      if subrec_container.ElementEditValues['Function'] = 'IsInList' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #1' then
          elementinteger := 5;
      if subrec_container.ElementEditValues['Function'] = 'IsInList' then
        if Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt) = 'Parameter #2' then
    ...
    ////////////////////////////////////////////////////////////////////////////
      ...
      subrec := rec.Add(elementpathstring, True);
		begin
			...
			subrec_container := previousrec;
			if (ansipos(containerpathstring, formatelementpath(Copy(subrec_container.Path, 8, MaxInt))) <> 0) then
			begin
				While (containerpathstring <> formatelementpath(Copy(subrec_container.Path, 8, MaxInt))) do
					subrec_container := AsContainer(subrec_container.Container);
				if not Assigned(subrec) then subrec := subrec_container.Elements[elementinteger];
				if ((subrec.Container.IndexOf(subrec) <> elementinteger)
...
//          AddMessage(IntToStr(elementinteger));
          try
            subrec := subrec_container.Assign(elementinteger, nil, False);
          except
            form.AddMessage('EXCEPTION in ElementAssign: Try MaxInt (2147483647) as elementinteger');
            Result := ExitSequence(form, rec, subrec_container, slstring[0], elementpathstring, elementvaluestring, IntToStr(elementinteger), elementisflag);
            Exit;
          end;
        end
				else subrec := subrec_container.Elements[elementinteger];
...
//      AddMessage(GetElementEditValues(rec, 'Record Header\Record Flags'));
      ...
      begin
        form.AddMessage('Warning Flags Integer is 0. Fix in corrosponding elementconversions file.');
        Result := ExitSequence(form, rec, subrec_container, slstring[0], elementpathstring, elementvaluestring, IntToStr(elementinteger), elementisflag);
        Exit;
      end;
//      AddMessage('STARTING');
...
//      AddMessage(elementpathstring);
      elementvaluestring := subrec_container.EditValue;
//      AddMessage('START');
...
//        AddMessage(elementvaluestring);
      ...
      try
        subrec_container.EditValue := elementvaluestring;
      except
        form.AddMessage('SUBREC_CONTAINER NOT EDITABLE');
        Result := ExitSequence(form, rec, subrec_container, slstring[0], elementpathstring, elementvaluestring, IntToStr(elementinteger), elementisflag);
        Exit;
      end;
      //AddMessage('ETYKOK');
      ...
      //Exit;
      elementvaluestring := '';
      subrec := subrec_container.ElementByPath[Copy(elementpathstring, (LastDelimiter('\', elementpathstring) + 1), MaxInt)];
//      AddMessage(GetElementEditValues(rec, 'Record Header\Record Flags'));
    end;
    ...
    begin
      subrec.Remove;
      subrec := iFileFallout4ESM.RecordByFormID[TwbFormID.FromCardinal($000138DE), True, True];
      subrec := AsContainer(subrec).ElementByPath['Layers\Alpha Layer'];
      subrec := subrec_container.Assign(elementinteger, subrec, False);
    end;
...
//      elementpathstring := 'Region Data Entries\Region Data Entry\RDMO';
    ...
    begin
      if not Assigned(subrec) then subrec := subrec_container.Add('RPLD', True);
    end;
...
//      AddMessage(Path(subrec));
      ...
      begin
        if subrec_container.ElementCount - 1 < StrToInt(Copy(elementpathstring, (LastDelimiter('#', elementpathstring) + 1), MaxInt)) then
          for k := subrec_container.ElementCount - 1 to (StrToInt(Copy(elementpathstring, (LastDelimiter('#', elementpathstring) + 1), MaxInt))) do
          begin
            subrec := subrec_container.Assign(MaxInt, Nil, False);
          end;
        if subrec_container.ElementCount - 1 > StrToInt(Copy(elementpathstring, (LastDelimiter('#', elementpathstring) + 1), MaxInt)) then
        begin
          subrec.Remove;
          subrec := subrec_container.Elements[StrToInt(Copy(elementpathstring, (LastDelimiter('#', elementpathstring) + 1), MaxInt))];
        end;
      end;
    end;
    ...
    begin
      if rec.Signature = 'ACHR' then
      begin
        subrec := rec.ElementByPath['NAME'].LinksTo;
        subrec := AsContainer(subrec).Add('ATTX', True);
      end;
    end;
    ...
    ////////////////////////////////////////////////////////////////////////////
		if Assigned(subrec) then
			if elementpathstring <> formatelementpath(Copy(subrec.Path, 8, MaxInt)) then
			begin
				if formatelementpath(Copy(subrec.Path, 8, MaxInt)) <> 'Record Header\Record Flags\NavMesh Generation' then
				begin
					form.AddMessage('subrec ' + elementpathstring + ' got assigned to wrong path ' + formatelementpath(Copy(subrec.Path, 8, MaxInt)));
          Result := ExitSequence(form, rec, subrec_container, slstring[0], elementpathstring, elementvaluestring, IntToStr(elementinteger), elementisflag);
          Exit;
...
//    AddMessage(Path(subrec_container));
    ...
    ////////////////////////////////////////////////////////////////////////////
		...
		begin
			...
			if Assigned(subrec_container) then
			begin
				if subrec_container.IndexOf(subrec_container.LastElement) > 0 then
          for k := 0 to subrec_container.IndexOf(subrec_container.LastElement) do
            if formatelementpath(Copy(subrec_container.Elements[k].Path, 8, MaxInt)) = elementpathstring then
              subrec := subrec_container.Elements[k];
			end;
			if not Assigned(subrec) then
			begin
        form.AddMessage('Failed to Assign Record');
        Result := ExitSequence(form, rec, subrec_container, slstring[0], elementpathstring, elementvaluestring, IntToStr(elementinteger), elementisflag);
        Exit;
				//Halt;
			end;
		end;
		if subrec.Path <> '' then previousrec := AsContainer(subrec);
    ...
    ////////////////////////////////////////////////////////////////////////////
      ...
      if Length(elementvaluestring) <> 8 then
      begin
        form.AddMessage('ERROR2: ' + elementvaluestring);
        form.AddMessage('Original LoadOrder: ' + originalloadorder);
        form.AddMessage('Missing Master');
        Result := 1;
        Exit;
      end;
      if not Assigned(
          FileByLoadOrder(
              Files, StrToInt('$' + Copy(elementvaluestring, 1, 2))).RecordByFormID[
                  TwbFormID.FromStr(elementvaluestring),
                  True, True])
      then
      begin
			  //AddMessage('Reference');
			  slReferences.Add(elementvaluestring + 
                         ';' + rec.LoadOrderFormID.ToString +         
                         ';' + rec._File.LoadOrder.ToString +
                         ';' + rec._File.FileName + 
                         ';' + subrec.Name + 
                         ';' + RetrieveIndexPath(form, subrec.PathName));
			  Exit;
      end
      else if StrToInt('$' + Copy(elementvaluestring, 1, 2)) <>
          FileByLoadOrder(
              Files, StrToInt('$' + Copy(elementvaluestring, 1, 2))).
                  RecordByFormID[TwbFormID.FromStr(elementvaluestring), True, True].
                      _File.LoadOrder then
        begin
		  	  //AddMessage('Reference');
		  	  slReferences.Add(elementvaluestring + 
                           ';' + rec.LoadOrderFormID.ToString + 
